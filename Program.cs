        ...
        static void Main(string[] args)
        {
            ...
            else
            {
                ...
                for(int i = 1; i < original.functions.Count - 1; i++)
                {
                    ...
                    if (originalStatements != decompedStatements) { statementErrors.Add("ERROR: " + original.functions[i].name + " statements do not match"); }
                    if (originalglobals != decompedglobals) { globalErrors.Add("ERROR: " + original.functions[i].name + " globals do not match"); }
                }
